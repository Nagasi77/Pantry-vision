...
function getPeriodLabel(
  ...
  if (period === "custom_hours")
    return `lebih dari ${customHours || "..."} jam`;
  ...
  const handleDeletePeriodic = async () => {
    ...
    if (deletePeriod === "custom_days") {
      ...
      if (!deleteCustomHours || deleteCustomHours <= 0) {
        alert("Masukkan jumlah jam yang valid (minimal 1).");
        return;
      }
      payload = {
        mode: "older_than",
        value: deleteCustomHours,
        unit: "hours",
      };
    } else if (deletePeriod === "all") {
      payload = { mode: "all" };
    } else if (deletePeriod.endsWith("h")) {
      const hours = parseInt(deletePeriod);
      payload = {
        mode: "older_than",
        value: hours,
        unit: "hours",
    ...
    try {
      ...
      alert(
        "Gagal menghapus riwayat scan secara periodik. Pastikan endpoint DELETE sudah tersedia dan mendukung mode older_than dengan unit hours/days."
      );
    } finally {
      ...
      <div className="bg-white border border-slate-100 rounded-[2rem] p-4 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-1">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            Menampilkan {shownStart}-{shownEnd} dari {filtered.length} sesi. Total objek terfilter: {filteredTotalItems}.
          </p>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-xs font-black text-slate-600 outline-none focus:ring-2 focus:ring-green-500/20"
            >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        ...
                        <td className="px-4 py-4 max-w-[260px]">
                          <div className="flex flex-wrap gap-1.5">
                            ...
                            {sess.detections.length === 0 && (
                              <span className="text-[10px] text-slate-400 italic">Tidak ada objek</span>
                            )}
                          </div>
                         </td>
                        <td className="px-4 py-4">
                                ...
                                {sess.detections.map((det, i) => (
                                  <div key={i} className="flex items-center gap-3 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                                    ...
                                    <div className="text-right">
                                      <span className={`text-[10px] font-black px-2.5 py-1 rounded-full border uppercase ${statusBadge(det.freshness_status)}`}>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 px-6 py-5 border-t border-slate-100 bg-slate-50/60">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              Halaman {safeCurrentPage} dari {totalPages}. Menampilkan {shownStart}-{shownEnd} dari {filtered.length} sesi.
            </p>
        ...
        >
          <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-100 flex items-start justify-between gap-4">
              ...
              <button onClick={closeDeleteModal} disabled={periodDeleting} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-50 rounded-xl transition-all disabled:opacity-50">
                <X size={20} />
              </button>
            </div>
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <button
                  type="button"
                  onClick={() => { setDeletePeriod("custom_days"); setDeleteCustomHours(""); }}
                  disabled={periodDeleting}
                  className={`sm:col-span-1 p-4 rounded-2xl border text-left transition-all ${
                ...
                <button
                  type="button"
                  onClick={() => { setDeletePeriod("custom_hours"); setDeleteStartDate(""); setDeleteEndDate(""); }}
                  disabled={periodDeleting}
                  className={`sm:col-span-1 p-4 rounded-2xl border text-left transition-all ${
              ...
              {deletePeriod === "custom_days" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-red-50/60 border border-red-100 rounded-2xl">
                  <label className="space-y-2">
                    <span className="flex items-center gap-1.5 text-[10px] font-black text-red-500 uppercase tracking-widest"><CalendarDays size={13} /> Dari Tanggal</span>
                    <input type="date" value={deleteStartDate} max={deleteEndDate || undefined} onChange={(e) => setDeleteStartDate(e.target.value)} disabled={periodDeleting} className="w-full bg-white border border-red-100 rounded-xl px-3 py-3 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-red-500/20" />
                  </label>
                  <label className="space-y-2">
                    <span className="flex items-center gap-1.5 text-[10px] font-black text-red-500 uppercase tracking-widest"><CalendarDays size={13} /> Sampai Tanggal</span>
                    <input type="date" value={deleteEndDate} min={deleteStartDate || undefined} onChange={(e) => setDeleteEndDate(e.target.value)} disabled={periodDeleting} className="w-full bg-white border border-red-100 rounded-xl px-3 py-3 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-red-500/20" />
                  </label>
              ...
              {deletePeriod === "custom_hours" && (
                <div className="p-4 bg-red-50/60 border border-red-100 rounded-2xl space-y-3">
                  <label className="block space-y-2">
                    <span className="flex items-center gap-1.5 text-[10px] font-black text-red-500 uppercase tracking-widest"><Clock size={13} /> Jumlah jam ke belakang (minimal 1)</span>
                    <input
                      type="number"
                      min="1"
                      value={deleteCustomHours}
                      onChange={(e) => setDeleteCustomHours(e.target.value === "" ? "" : Number(e.target.value))}
                      disabled={periodDeleting}
                      placeholder="Contoh: 72"
                      className="w-full bg-white border border-red-100 rounded-xl px-4 py-3 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-red-500/20"
                    />
                  </label>
                  <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest">
                    Data yang dihapus: {getPeriodLabel(deletePeriod, undefined, undefined, deleteCustomHours)}.
                  </p>
                </div>
              )}

              {/* Info ringkasan untuk opsi non-custom */}
              {deletePeriod && deletePeriod !== "custom_days" && deletePeriod !== "custom_hours" && (
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Data yang akan dihapus: {getPeriodLabel(deletePeriod)}.</p>
                </div>
              )}
            </div>
            <div className="p-6 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={handleDeletePeriodic} disabled={periodDeleting || !deletePeriod} className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-4 bg-red-500 hover:bg-red-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all disabled:opacity-60 disabled:cursor-not-allowed">
        ...
        >
          <div className="bg-white rounded-[3rem] overflow-hidden shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                <p className="font-black text-slate-800 text-sm">{formatDate(selectedSession.scanned_at)}</p>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{formatTime(selectedSession.scanned_at)} · {selectedSession.device_source}</p>
              </div>
